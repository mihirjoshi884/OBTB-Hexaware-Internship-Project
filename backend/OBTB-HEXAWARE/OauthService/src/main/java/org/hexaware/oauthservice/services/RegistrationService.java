...
public class RegistrationService {
    ...
    private SecurityQuestion  storeHashQuestion(List<HashMap<String, String>> SecQA, UUID currentUserId){
        List<String> hashedQAPairs = SecQA.stream()
                .map(map ->{
                    String question = map.keySet().iterator().next();
                    String answer = map.get(question);
                    String concatenatedString = question + answer;
                    try {
                        return createHash(concatenatedString);
                    } catch (Exception e) {
